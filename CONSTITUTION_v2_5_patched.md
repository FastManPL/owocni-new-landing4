...
| C6.3 | **Layout-settle refresh (sekcje z ScrollTrigger).** Sekcja która tworzy ScrollTriggery (trigger/start/end) MUSI po zbudowaniu ST wywołać **opóźniony** `scrollRuntime.requestRefresh('layout-settle')` — np. `setTimeout(..., 1000 ms)` po `fonts.ready` / po utworzeniu ostatniego ST (przy ciężkiej treści nad sekcją: 800–1200 ms). Timer zapisany w `timerIds` i czyszczony w `kill()`. Szczegóły: `docs/SCROLL_TRIGGER_GUIDELINES.md`. | Przy pierwszym refresh ST pozycje triggera są mierzone w momencie wywołania. Jeśli nad sekcją jest dużo treści (długi spacer, obrazy ładujące się późno), layout dokumentu ustala się z opóźnieniem → start/end policzone za wcześnie → animacja „startuje za wcześnie” lub kończy się zanim sekcja wejdzie na ekran. Opóźniony refresh po ~1000 ms daje przeglądarce czas na ustabilizowanie layoutu. |
...
| J13 | **GSAP-SSR-01 — `gsap.registerPlugin()` wyłącznie po stronie klienta.** Next.js App Router pre-renderuje Client Components na serwerze (SSR). `gsap.registerPlugin()` na module top-level pliku TSX odpala podczas tego pre-renderingu — `window`/`document` nie istnieją → błąd lub hydration mismatch. Jedyne dozwolone miejsca: (a) wewnątrz `useGSAP(() => { gsap.registerPlugin(...) })` — najczystsze, scope GSAP widzi od razu; (b) opakowane w `if (typeof window !== 'undefined') { gsap.registerPlugin(...) }` — gdy plugin musi być dostępny przed wywołaniem useGSAP. **ZAKAZ: `gsap.registerPlugin()` na module top-level bez jednego z tych guardy.** Naruszenie = STOP w P3. | SSR pre-render Client Component = środowisko Node.js bez window. registerPlugin bez guardu = crash lub hydration mismatch. To jest jeden z najczęstszych "works locally, breaks on Vercel" błędów Next.js 16. |
