...
| C6.3 | **Layout-settle refresh (sekcje z ScrollTrigger).** Sekcja która tworzy ScrollTriggery (trigger/start/end) MUSI po zbudowaniu ST wywołać **opóźniony** `scrollRuntime.requestRefresh('layout-settle')` — np. `setTimeout(..., 300–500 ms)` po `fonts.ready` / po utworzeniu ostatniego ST. Timer zapisany w `timerIds` i czyszczony w `kill()`. Szczegóły: `docs/SCROLL_TRIGGER_GUIDELINES.md`. | Przy pierwszym refresh ST pozycje triggera są mierzone w momencie wywołania. Jeśli nad sekcją jest dużo treści (długi spacer, obrazy ładujące się późno), layout dokumentu ustala się z opóźnieniem → start/end policzone za wcześnie → animacja „startuje za wcześnie” (progress > 0 zanim sekcja wejdzie na ekran). Opóźniony refresh po ~400 ms daje przeglądarce czas na ustabilizowanie layoutu. |
