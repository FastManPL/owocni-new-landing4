...
function init(container: HTMLElement): { kill: () => void } {
  ...
  function _saturateHSL(h: number, s: number, l: number): [number, number, number] {
  ...
  function _bakeHSL(h: number, s: number, l: number, a: number) { const[h2,s2,l2]=_saturateHSL(h,s,l); return `hsla(${h2},${s2}%,${l2}%,${a})`; }
  function _bakePrefix(h: number, s: number, l: number) { const[h2,s2,l2]=_saturateHSL(h,s,l); return 'hsla('+h2+','+s2+'%,'+l2+'%,'; }

  interface PrecomputedLineColors {
    areaTop: string; areaMid: string; areaEnd: string;
    strokeStart: string; strokeMid: string; strokeEnd: string;
    shadow: string;
  }
  ...
  interface PrecomputedFlareColors {
    glow0: string; glow55: string;
    ringFull: [string, string];
    midFull: string; innerFull: string;
  }
  ...
  interface BezierSeg {
    ...
    c2x: number; c2y: number; ex: number; ey: number;
  }
  ...
  function catmullRomSegs(pts: number[][],tension=0.5): BezierSeg[] {
    ...
    for(let i=0;i<pts.length-1;i++){
      ...
      const x0=p1?.[0]??0,y0=p1?.[1]??0,x1=p2?.[0]??0,y1=p2?.[1]??0,x0p=p0?.[0]??0,y0p=p0?.[1]??0,x3=p3?.[0]??0,y3=p3?.[1]??0;
      segs.push({
        sx:x0,sy:y0,
        c1x:x0+(x1-x0p)/(6*tension),c1y:y0+(y1-y0p)/(6*tension),
        c2x:x1-(x3-x0)/(6*tension),c2y:y1-(y3-y0)/(6*tension),
        ex:x1,ey:y1
      });
    }
    return segs;
  ...
  function bezierSegsFromNodes(nodes: PathNode[], W: number, H: number, yOff: number): BezierSeg[] {
    ...
    for (let i = 0; i < nodes.length - 1; i++) {
      ...
      const ax0 = (n0.a[0] ?? 0)*W, ay0 = ((n0.a[1] ?? 0)+yOff)*H;
      const ax1 = (n1.a[0] ?? 0)*W, ay1 = ((n1.a[1] ?? 0)+yOff)*H;
      const c1x = ((n0.a[0] ?? 0)+(n0.ho[0] ?? 0))*W, c1y = ((n0.a[1] ?? 0)+(n0.ho[1] ?? 0)+yOff)*H;
      const c2x = ((n1.a[0] ?? 0)+(n1.hi[0] ?? 0))*W, c2y = ((n1.a[1] ?? 0)+(n1.hi[1] ?? 0)+yOff)*H;
  ...
  function drawOrgLine(ctx: CanvasRenderingContext2D, segs: BezierSeg[], active: number, H: number, _W: number, c: OrgConfig, clr: PrecomputedLineColors, strokeGrad: CanvasGradient) {
    ...
    if(active<=0){_lineHead.lx=s0.sx;_lineHead.ly=s0.sy;return _lineHead;}
  ...
  function drawOrgFlare(ctx: CanvasRenderingContext2D, x: number, y: number, progress: number, t: number, c: OrgConfig, fc: PrecomputedFlareColors) {
  ...
  function drawGridLines(ctx: CanvasRenderingContext2D, W: number, H: number, pp: number) {
  ...
  const orgOverlay = container.querySelector<HTMLCanvasElement>('#organic-overlay');
  ...
  let orgCachedPSegs: BezierSeg[] | null=null, orgCachedSSegs: BezierSeg[] | null=null;
  let orgStrokeGradP: CanvasGradient | null=null, orgStrokeGradS: CanvasGradient | null=null;
  ...
  const orgOverlayWrapper = container.querySelector<HTMLDivElement>('#organic-overlay-wrapper');
  function resizeOrgCanvas() {
    if(!orgOverlay||!orgCtx||!faktyBlock)return;
    const wrap = orgOverlayWrapper;
    if (wrap && faktyDom) {
      wrap.style.top = faktyDom.offsetTop + 'px';
      wrap.style.left = faktyDom.offsetLeft + 'px';
      wrap.style.width = faktyDom.offsetWidth + 'px';
      wrap.style.height = faktyDom.offsetHeight + 'px';
    }
  ...
  // TUNNEL ENGINE
  // ══════════════════════════════════════════════════════════
  const tunnelCanvas = container.querySelector<HTMLCanvasElement>('#fakty-tunnel');
  ...
  function kill() {
    ...
    cleanups.forEach(fn=>{ try{fn();}catch(e){/* cleanup guard */} });
    timerIds.forEach(t=>{
      try{
        ...
        else if(t.type==='idle'&&typeof window.cancelIdleCallback==='function')window.cancelIdleCallback(id);
      }catch(e){/* cleanup guard */}
    });
    observers.forEach(obs=>{ try{obs?.disconnect?.();}catch(e){/* cleanup guard */} });
  ...
  return (
    <section id="fakty-section" ref={rootRef}>
      <div className="title-block" id="fakty-block">
        ...
        <div id="organic-overlay-wrapper" className="organic-overlay-wrapper">
