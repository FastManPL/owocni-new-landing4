...
function init(container: HTMLElement): { kill: () => void } {
  ...
  function buildPhase1() {
    ...
    tlRotation.to(
      row1Chars,
      { ease: "power1", stagger: 0.07, rotationX: 0, z: 0, duration: 0.3 },
      0.52,
    );
    ...
    tlOpacity.to(
      row1Chars,
      { opacity: 1, ease: "power2.in", stagger: 0.05, duration: 0.12 },
      0,
    );
  ...
  function catmullRomSegs(pts: number[][], tension = 0.5): BezierSeg[] {
    ...
    for (let i = 0; i < pts.length - 1; i++) {
      const p0 = pts[Math.max(i - 1, 0)] ?? [0, 0],
        p1 = pts[i] ?? [0, 0],
        p2 = pts[i + 1] ?? [0, 0],
        p3 = pts[Math.min(i + 2, pts.length - 1)] ?? [0, 0];
      const x0 = p1[0] ?? 0,
        y0 = p1[1] ?? 0,
        x1 = p2[0] ?? 0,
        y1 = p2[1] ?? 0;
      const dx0 = (p2[0] ?? 0) - (p0[0] ?? 0),
        dy0 = (p2[1] ?? 0) - (p0[1] ?? 0),
        dx1 = (p3[0] ?? 0) - (p1[0] ?? 0),
        dy1 = (p3[1] ?? 0) - (p1[1] ?? 0);
      segs.push({
        sx: x0,
        sy: y0,
        c1x: x0 + dx0 / (6 * tension),
        c1y: y0 + dy0 / (6 * tension),
        c2x: x1 - dx1 / (6 * tension),
        c2y: y1 - dy1 / (6 * tension),
        ex: x1,
        ey: y1,
      });
    }
  ...
  function bezierSegsFromNodes(
    ...
    for (let i = 0; i < nodes.length - 1; i++) {
      const n0 = nodes[i]!,
        n1 = nodes[i + 1]!;
      const ax0 = (n0.a[0] ?? 0) * _W,
        ay0 = ((n0.a[1] ?? 0) + yOff) * H;
      const ax1 = (n1.a[0] ?? 0) * _W,
        ay1 = ((n1.a[1] ?? 0) + yOff) * H;
      const c1x = ((n0.a[0] ?? 0) + (n0.ho[0] ?? 0)) * _W,
        c1y = ((n0.a[1] ?? 0) + (n0.ho[1] ?? 0) + yOff) * H;
      const c2x = ((n1.a[0] ?? 0) + (n1.hi[0] ?? 0)) * _W,
  ...
  function drawOrgLine(
    ...
    const s0 = segs[0];
    if (active <= 0 || !s0) {
      _lineHead.lx = s0?.sx ?? 0;
      _lineHead.ly = s0?.sy ?? 0;
      return _lineHead;
    }
    ctx.beginPath();
    ctx.moveTo(s0.sx, H);
  ...
  function buildTunnel() {
    ...
    tunnelST = ScrollTrigger.create({
      ...
      onUpdate: function (self) {
        tunnelDraw(Math.floor(self.progress * T_SPEED * (T_STEPS - 1)));
      },
    });
  ...
  Promise.all([
    ...
    .then(async () => {
      ...
      timerIds.push({
        type: "timeout",
        id: () => layoutSettleTimerId as number | null,
      });
