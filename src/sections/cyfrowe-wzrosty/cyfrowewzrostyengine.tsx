 ...
 * istniejącego DOM-u (GSAP, ScrollToPlugin, spring physics ticker, 94× layer
 * creates w init loop). Cały statyczny markup sekcji (4× H3 tile-heading,
 * tile-body, ribbons, labels, videos z `preload="none"`) renderuje wrapper
 * `CyfroweWzrostySectionWrapper` (SSR). Ten komponent `return null`.
 *
 * Kontrakt z wrapperem:
 *  - `rootRef` wskazuje na `<section id="cyfrowe-wzrosty-section">` (scope dla
 *    useGSAP i argument dla init()).
 *  - Wrapper ma już wszystkie selektory, których używa init(): `.tile`,
 *    `.ribbon-name`, `.tile-heading`, `.tile-body`, `#cyfrowe-wzrosty-*`,
 *    `[data-warm-video]`.
 *  - CSS importuje wrapper (nie ten plik) — SSR HTML ma styl od pierwszej klatki.
 */
type CyfroweWzrostyEngineProps = {
...
// ═══════════════════════════════════════════════════════════════
// React Component — Engine (loaded via dynamic import, ssr:false)
// ═══════════════════════════════════════════════════════════════
export function CyfroweWzrostyEngine({ rootRef }: CyfroweWzrostyEngineProps) {
  useGSAP(() => {
  ...
  // SSR markup sekcji żyje w `CyfroweWzrostySectionWrapper` (wrapper client). Engine
  // renderuje `null` — wszystkie selektory w init() działają na DOM-ie wrappera
  // dzięki `rootRef` przekazanemu przez prop.
  return null;
}
