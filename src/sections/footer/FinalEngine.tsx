...
function getBlankTex(){
  ...
  cv.getContext('2d').fillStyle='white'; cv.getContext('2d').fillRect(0,0,4,4);
...
function warmup(){
  ...
  _ric(function(){
    ...
    // ── FAZA 3 (rAF): tekstury zegara + compileAsync ─────────────────────────
    ...
    }, 2000); // end _ric faza 3 (timeout 2s)
...
function resume(){
  ...
  if(window.innerWidth<768){ gsap.ticker.fps(30); } // O9: 30fps mobile
  if(!clockIntervalId){
    _startClockInterval();
    state=CLK.CLOCK;
    nextCall=gsap.delayedCall(CYCLE,showDay);
