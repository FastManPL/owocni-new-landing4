...
function init(container: HTMLElement | null): { kill: () => void; pause: () => void; resume: () => void } {
  ...
  */
  ...
  (function bookFrames() {
    ...
    const FRAME_BASE = '/books/Ksiazka-Klatki/frame-';
    ...
    /* --- Load real frame images from /books/Ksiazka-Klatki --- */
    function loadFrameImage(idx: number, onAllLoaded: () => void): void {
      const num = String(idx + 1).padStart(3, '0');
      const src = FRAME_BASE + num + FRAME_EXT;
      const img = new Image();
      img.onload = function() {
        frames[idx] = img;
        loaded[idx] = true;
    ...
    function startPreload() {
      ...
      let allLoadedCalled = false;
      function whenAllLoaded() {
        if (allLoadedCalled) return;
      ...
      for (let i = 0; i < loadOrder.length; i++) {
        ...
        loadFrameImage(idx, whenAllLoaded);
      }
    }
        ...
        <div className="container">
          ...
          <div className="cs-floor cs-floor--images">
            <div className="cs-floor__left">
              <video
                className="cs-video--banner"
                src="/books/banner-konwersja-strony.mp4"
                autoPlay
                muted
                loop
                playsInline
                aria-label="Konwersja stron"
              />
            </div>
