 ...
 *
 * Ten plik zawiera CAŁĄ ciężką logikę interaktywną sekcji Wyniki:
 *  - GSAP ScrollTrigger + zoom timeline (mockup layer + tlo + overlay)
 ...
 *  - useEffect dla reduced-motion + Wistia play
 *  - Conditional render `<wistia-player>` po `wistiaActivated && popupOpen`
 *  - Cały SEO-critical markup (h2, sub, cta, footnote, image, video mockup, popup)
 *
 * Komunikacja engine → wrapper odbywa się przez callbacki `onPopupOpen`/
 * `onPopupClose` przekazane w props (engine nie tyka React state bezpośrednio).
 *
 * Engine renderuje `null` — wszystko jest DOM-manipulation w `container`
 * (przekazany jako `rootRef.current`). `ssr: false` dla engine = zero SSR
 * overhead bez utraty SEO (markup jest w wrapperze).
 */

type WynikiEngineProps = {
...
export function WynikiEngine({ rootRef, onPopupOpen, onPopupClose }: WynikiEngineProps) {
  useGSAP(
    () => {
      ...
      if (!el) {
        if (process.env.NODE_ENV !== 'production') {
          throw new Error('[P3] WynikiEngine: rootRef.current is null — ref not attached to <section>.');
        }
        return;
      }
      const inst = init(el, { onPopupOpen, onPopupClose });
      return () => inst?.kill?.();
    },
    { scope: rootRef },
  );
