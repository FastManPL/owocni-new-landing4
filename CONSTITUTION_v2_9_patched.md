...
| **Shared Core Layer** | Zatwierdzone globalne usługi: scrollRuntime, consent/analytics, moduleLoader, vitals, design tokens, next/font, webglBroker. |
...
| **webglBroker** | Cienki singleton w `src/lib/webglBroker.ts`. Część Shared Core Layer. Zarządza wyłącznie: wspólnym profilem jakości (`getWebGLProfile()`), kolejką warmup GPU, obsługą `webglcontextlost`, limitem sekcji HOT oraz globalnym `pause-all` przy `document.hidden`. NIE zarządza renderowaniem — każda sekcja ma własny `WebGLRenderer` i własny canvas. |
| **WebGL Profile** | Wynik `getWebGLProfile()` — jeden z: `none` / `low` / `normal` / `high`. Obliczany raz przy init na podstawie capability tier + opcjonalnego hintu GPU. Determinuje: clamp DPR, antialias, `powerPreference`, intensywność efektów, fallback mode. Wszystkie sekcje WebGL na stronie używają tego samego profilu. |
| **WebGL Lifecycle** | Cztery stany sekcji z WebGL: `COLD` (kod pobrany, renderer nie istnieje), `WARM` (renderer stworzony, shadery i tekstury przygotowane, loop nie działa), `HOT` (sekcja renderuje), `OFF` (loop zatrzymany; po timeout — pełny dispose → COLD). |
...
| # | Zasada | Dlaczego |
|---|--------|----------|
| B1 | Każda sekcja to zamknięty, samowystarczalny React component. Zero importów krzyżowych między sekcjami. **Wyjątek:** zatwierdzony Shared Core Layer (scrollRuntime, consent/analytics, moduleLoader, vitals, design tokens, next/font, webglBroker). | Wymienialność sekcji. Shared Core to infrastruktura, nie zależność między sekcjami. |
...
| B4.1 | **HTML-FONTSIZE-01 — stały `html { font-size: 16px }` w `globals.css`.** Wszystkie sekcje zakładają `html { font-size: 16px }` jako bazę REM — stałe, globalne, jawne. Sekcja NIE deklaruje własnego `html { font-size }` ani `:root { font-size }`. Responsive scaling typografii realizowany wyłącznie przez `clamp()` z `vw` jako środkowym argumentem. Naruszenie w P2A/P3 = STOP. **Dlaczego jawne 16px zamiast pominięcia:** domyślne 16px przeglądarki mogą być nadpisane przez accessibility settings użytkownika — `html { font-size: 16px }` zamraża bazę i gwarantuje że PREVIEW.html = produkcja = identyczne proporcje. | Responsywny `html { font-size }` z breakpointami (kalkulacje vw/rem) powoduje visual drift między PREVIEW a wdrożeniem — sekcja zaakceptowana w PREVIEW wygląda inaczej po integracji. Stała baza eliminuje tę klasę błędów. |
...
| B7.4 | **Geometry sensitivity contract.** Jeśli `integrationNotes.sensitiveTo` nie jest puste (np. `['geometry-above']`), manifest MUSI zawierać co najmniej jeden z mechanizmów ochronnych: (a) `refreshSignals` niepuste — P3 konsumuje mechanicznie przez C6.3; (b) `stack-harness: 'wymagany'` — P2B generuje harness; (c) `geometryRefresh: 'self'` — sekcja sama woła `requestRefresh`; (d) `specialNotes` z opisem konkretnego mechanizmu. **`sensitiveTo` niepuste + żaden mechanizm = BLOCKER w P2B.** WAŻNA DISTINKCJA: B7.4 NIE wymaga automatycznie `geometryMutable: true`. To są dwa różne byty: `geometryMutable` = sekcja **zmienia** geometrię dokumentu (accordion, expand); `sensitiveTo` = sekcja jest **zależna** od geometrii otoczenia (ST mierzone względem elementów powyżej). Sekcja może być jedno bez drugiego — case „Fakty": `geometryMutable: false` + `sensitiveTo: ['geometry-above']` = sekcja nie zmienia własnej wysokości, ale jej ScrollTriggery są wrażliwe na zmiany geometrii nad nią. Bez mechanizmu ochronnego to jest tykająca bomba. | Manifest który „wie" że sekcja jest wrażliwa ale nie daje żadnego narzędzia jest gorszy niż brak deklaracji — daje fałszywe poczucie bezpieczeństwa. Integrator który czyta `geometryMutable: false` działa bez hooka. Sekcja z `sensitiveTo: ['geometry-above']` bez mechanizmu = broken ScrollTriggery po każdej zmianie geometrii powyżej, bez możliwości debugowania (integrator myśli że sekcja jest bezpieczna geometrycznie). |
| B7.5 | **Section Position Invariance (`POS-ROOT-01`).** ScrollTriggery opisujące wejście / wyjście / aktywację sekcji względem viewportu MUSZĄ mieć `trigger` wskazujący root sekcji (`container` / `#[slug]-section`) oraz `start`/`end` w notacji element↔viewport (`'top bottom'`, `'bottom top+=20%'` itp.). **ZAKAZ:** document-relative liczby jako kotwiczenie pozycji sekcji (`start: 800`, `end: 1200`, `'+=800'` bez członu viewport). Dla sekcji `integrationNotes.sensitiveTo: ['geometry-above']` rekomendowanym mechanizmem ochronnym są `refreshSignals` z wartościami `'section-in-view'` i `'layout-settle'` (B7.4 + C6.3) oraz stack-harness z testem INT-04 w P2B. **NOTA INTERPRETACYJNA:** wewnętrzny element jako trigger = DOZWOLONY dla animacji lokalnych wewnątrz widocznej już sekcji; ZAKAZ jako semantyka „sekcja wchodzi na ekran". Viewport-relative string z offsetem px (np. `'top bottom-=80'`) nie jest document-relative — wymaga REVIEW, nie jest automatycznym STOP. **P3:** `'section-in-view'` i `'layout-settle'` są legalnymi wartościami `refreshSignals` — P3 konsumuje je mechanicznie z Patch I (tak samo jak `'fonts-ready'`, `'assets-loaded'`, `'video-metadata'`). Mechanizmy z ST-REFRESH-01 zachowują swoje miejsce w `init(container)` po translacji vanilla→TSX — nie są wyciągane do osobnych React `useEffect`. | Sekcja musi zachowywać się identycznie na górze strony, po 200vh spacerze i w środku LP. Root trigger + viewport-relative start/end eliminują zależność od pozycji dokumentu. `'section-in-view'` odpala refresh gdy sekcja jest już w stabilnym otoczeniu (lazy load powyżej zakończony, hydration sąsiadów zakończona). `'layout-settle'` (~1000ms) zabezpiecza przed post-entry zmianami layoutu (fonty, obrazki bez explicit dimensions). Oba razem tworzą dwuwarstwową ochronę — sam timer bez IO = odpala nawet gdy user nie doscrolluje do sekcji; samo IO bez timera = może przegapić post-entry layout changes. |
...
| C2b | **Kinetic section — świadomy wyjątek singleton bus.** Sekcja `kinetic` (BridgeSequence) używa `window.__kineticBus` jako wewnętrznego mostu między 4 IIFE (Particle, Tunnel, Cylinder, Explosion). Jest to świadoma decyzja architektoniczna z lutego 2026 — refaktor wymaga przebudowy ~2000 linii z wysokim ryzykiem regresu wizualnego. **ZAKAZ modyfikacji `window.__kineticBus` bez osobnej sesji roboczej z pełnym testem na mobile i desktop.** `kill()` sekcji czyści `window.__kineticBus = null` — brak wycieku między sesjami. Fabryka: ENT-JS-09 dla tej sekcji = CANNOT-FIX, świadoma decyzja właściciela. | Sekcja jest singletonem na stronie — window.* bus nie powoduje wycieku w praktyce. Refaktor możliwy w przyszłości jako osobna sesja z backupem. |
...
| C5 | **`ScrollTrigger.config({ ignoreMobileResize: true })` + orientationchange z double rAF settle.** `ignoreMobileResize: true` blokuje resize events wywołane przez mobilną klawiaturę (nie resetuje scrolla przy wpisywaniu). Rotacja ekranu to INNA klasa zdarzenia — geometria faktycznie się zmienia, ST musi to wiedzieć. Implementacja w `SmoothScrollProvider.tsx`: `window.addEventListener("orientationchange", handler)` gdzie handler używa **double rAF** — rotacja "osiada" dopiero po 1–2 klatkach: `requestAnimationFrame(() => requestAnimationFrame(() => scrollRuntime.requestRefresh("orientationchange-settle")))`. Implementacja jest w CORE (SmoothScrollProvider) — sekcje NIE dodają własnych orientationchange listenerów. | ignoreMobileResize redukuje jumpy na mobile, ale orientationchange wymaga ręcznego refresh z settle. Pojedynczy rAF łapie event przed ukończeniem layout-u po rotacji. Double rAF gwarantuje że browser zakończył layout = poprawna geometria = dobry refresh. |
...
| C6.3 | **Async boundary tworzący ScrollTriggery wymaga post-settle `requestRefresh`.** Jeśli sekcja tworzy ScrollTriggery lub mierzy geometrię wewnątrz async callbacku (`document.fonts.ready.then()`, `setTimeout`, `Promise`, `IntersectionObserver` callback, `loadedmetadata`, `decode()`) — MUSI wywołać `scrollRuntime.requestRefresh(reason)` po zakończeniu tej granicy. Mechanizm błędu: async callback odpala po hydration Next.js — geometria strony między momentem montowania komponentu a resolve callbacku mogła się zmienić (lazy load powyżej, hydracja sąsiednich sekcji, fonty zmieniające layout). GSAP mierzy pozycje raz przy inicjalizacji. Wszystkie zmiany po tym pomiarze są dla niego niewidoczne dopóki refresh nie odpali. Forma settle zależy od kontekstu — reguła wymaga **post-settle refresh przez broker**, nie jednego konkretnego kształtu kodu: `fonts-ready` → `requestAnimationFrame(() => requestRefresh('fonts-ready-settle'))` / `loadedmetadata` → bezpośrednio w handlerze / `decode()` → w `.then()` / złożone settle → double rAF. **Powiązanie z manifestem:** P2B wpisuje async boundaries do `refreshSignals` — P3 konsumuje je mechanicznie (tabela mapowania w P3_PATCH Patch I). P3 nie zgaduje — manifest mówi mu co jest. `refreshSignals: []` przy istniejącym async boundary tworzącym ST = BLOCKER w P2B: MANIFEST INCOMPLETE. | `useGSAP` nie wykonuje automatycznego `ScrollTrigger.refresh()` po async callbackach — obsługuje tylko synchroniczny cleanup/revert przy unmount. Async callback poza synchronicznym flow jest dla GSAP niewidoczny. Efekt: ST zbudowane na „stale coordinates" — sekcja wydaje się działać w PREVIEW (lokalny serwer, szybki dysk, brak hydracji sąsiadów) ale sypie się w produkcji (hydration drift, lazy load powyżej). Manifest z `refreshSignals` + mechaniczne mapowanie P3 = deterministyczna ochrona bez „mądrości" P3. |
| C7 | **CSS Lenisa: `import "lenis/dist/lenis.css"` w `src/app/layout.tsx`.** (Od Lenis 1.1+: ścieżka `lenis/lenis.css`; starsze wersje używały `lenis/dist/lenis.css` — sprawdź wersję w `package.json`.) Import MUSI być w `layout.tsx`, nie w `globals.css` (zachowanie kolejności importów przy Turbopack). **STOP przed wdrożeniem:** sprawdź czy `globals.css` lub `layout.tsx` ma `html { height: 100% }` lub `body { height: 100% }` — Lenis CSS ustawia `html { height: auto }` co może powodować layout shift. Bez importu CSS Lenisa: dziwne zachowania na mobile (nested scroll, modale, overscroll-behavior). | Bez lenis.css: mobile nested scroll nie blokuje, overscroll-behavior nieokreślony, modale mogą przewijać tło. Kolejność importów jest deterministyczna w layout.tsx. |
...
| G11.1 | **`getWebGLProfile()` — jeden profil jakości WebGL dla całej strony.** Funkcja w `src/lib/webglBroker.ts`, obliczana raz przy init. Zwraca: `none` / `low` / `normal` / `high`. Bazuje na capability tier (G11) jako głównym sygnale, uzupełnionym opcjonalnie o hint GPU z `WEBGL_debug_renderer_info` extension — jeśli extension jest dostępna (może być wyłączona lub ograniczona przez przeglądarkę). **Hint GPU:** `gl.getExtension('WEBGL_debug_renderer_info')` → `UNMASKED_RENDERER_WEBGL` → string zawierający np. "Intel Iris", "Apple M", "Mali", "Adreno". Używany jako pomocniczy sygnał, nie jako jedyne źródło prawdy — system musi działać poprawnie gdy extension jest niedostępna. **Konsekwencje profilu dla każdej sekcji WebGL:** `none` → fallback (statyczny obraz lub ukrycie canvas), `low` → `renderer.setPixelRatio(1)`, `antialias: false`, `powerPreference: 'low-power'`, uproszczone efekty, `normal` → `setPixelRatio(Math.min(dpr, 1.5))`, `antialias: true`, `powerPreference: 'default'`, `high` → `setPixelRatio(Math.min(dpr, 2))`, pełne efekty. **ZAKAZ** stosowania różnych profili w różnych sekcjach tej samej strony — wszystkie sekcje dostają wynik jednego `getWebGLProfile()` wywołania. Zakaz UA/platform heurystyk jako jedynego sygnału — bazuj na capability, nie na nazwie platformy lub OS. | Bez wspólnego profilu: sekcja A renderuje `high`, sekcja B renderuje `low` — niespójny UX i niespójna presja na GPU. Hint GPU uzupełnia capability tier bo `hardwareConcurrency` nie mówi nic o jakości GPU — Mac M2 i Mac Intel Iris 2017 mają identyczne CPU cores, radykalnie różną GPU. `WEBGL_debug_renderer_info` jest szeroko dostępna ale jako extension może być celowo wyłączona (privacy, sandboxing) — nie może być jedynym decydentem. |
...
| J13 | **GSAP-SSR-01 — `gsap.registerPlugin()` wyłącznie po stronie klienta.** Next.js App Router pre-renderuje Client Components na serwerze (SSR). `gsap.registerPlugin()` na module top-level pliku TSX odpala podczas tego pre-renderingu — `window`/`document` nie istnieją → błąd lub hydration mismatch. Jedyne dozwolone miejsca: (a) wewnątrz `useGSAP(() => { gsap.registerPlugin(...) })` — najczystsze, scope GSAP widzi od razu; (b) opakowane w `if (typeof window !== 'undefined') { gsap.registerPlugin(...) }` — gdy plugin musi być dostępny przed wywołaniem useGSAP. **ZAKAZ: `gsap.registerPlugin()` na module top-level bez jednego z tych guardy.** Naruszenie = STOP w P3. **ROZSZERZENIE — bundler-first:** Sekcja React/TSX nie może polegać na `window.*` jako źródle ŻADNEJ biblioteki importowanej modułowo. Zakaz dotyczy wzorca „w PREVIEW było globalne CDN → w TSX też sprawdzę `window.xxx`". Najczęstsze naruszenia: `window.gsap` → `import gsap from 'gsap'`; `window.ScrollTrigger` → `import { ScrollTrigger } from 'gsap/ScrollTrigger'`; `window.THREE` → `import * as THREE from 'three'`; `window.Lenis` → `import Lenis from 'lenis'`. W bundlerze te wartości na `window` są `undefined` — warunek `if (window.gsap)` jest zawsze `false` = martwy kod = biblioteka/animacja nigdy nie odpala. Grep gate w P3: `rg "window\.gsap\|window\.ScrollTrigger\|window\.THREE\|window\.Lenis" src/sections/[slug] -S` → każde trafienie = BLOCKER. | SSR pre-render Client Component = środowisko Node.js bez window. registerPlugin bez guardu = crash lub hydration mismatch. To jest jeden z najczęstszych "works locally, breaks on Vercel" błędów Next.js 16. Bundler-first: GSAP, Three.js, Lenis ładowane przez npm — `window.*` nie jest ustawiane przez bundler. `window.gsap` w Next.js = `undefined`. Warunek `if (window.gsap)` zawsze false = animacja nie odpala bez błędu = najtrudniejszy do debugowania typ regresu. |
| J14 | **GSAP-SSR-02 — escape hatch gdy import pluginu (nie registerPlugin) crashuje SSR.** Jeśli sam statyczny import pluginu (`import { ScrollTrigger } from 'gsap/ScrollTrigger'`) powoduje `"window is not defined"` w stacktrace w `node_modules/gsap/...` — plugin ma window-zależne side-effecty przy imporcie. Rozwiązanie: ustaw w manifeście sekcji `perf.loading.dynamicImport: true` + `perf.loading.clientOnly: true` → SectionsClient.tsx importuje sekcję przez `next/dynamic(..., { ssr: false })` = wyłącza SSR pre-render całej sekcji. **ZAKAZ: `require()` jako workaround** — w ESM/Turbopack jest niepewny. **ZAKAZ: async `import()` wewnątrz `useGSAP`** — useGSAP cleanup odpala przed async IIFE → zombie ScrollTrigger + memory leak. W praktyce: GSAP ScrollTrigger NIE wykonuje window-kodu przy statycznym imporcie — J13 zamyka 99% przypadków. J14 stosuj TYLKO gdy stacktrace wskazuje na import, nie registerPlugin. | Turbopack + ESM nie toleruje `require()` w module-scope. async IIFE w useGSAP łamie cleanup contract. `ssr: false` to jedyna deterministyczna ścieżka dla modułów z window-side-effects. |
| J15 | **WebGL Lifecycle — cztery stany, per-sekcja renderer, cienki broker.** Sekcja z Three.js/WebGL zarządza własnym `WebGLRenderer` i własnym `<canvas>`. Globalnie współdzielony jest tylko `webglBroker` (polityka, nie rendering). **Cztery stany sekcji WebGL:** (1) `COLD` — chunk Three.js pobrany przez moduleLoader, renderer NIE istnieje, zero zasobów GPU; (2) `WARM` — renderer stworzony (IntersectionObserver rootMargin ~100% przed viewport), `renderer.compileAsync(scene, camera)` odpalony, `renderer.initTexture(tex)` dla każdej tekstury, loop NIE działa — zero CPU/GPU overhead; (3) `HOT` — sekcja w viewport, loop startuje, shadery już skompilowane = zero first-render stall; (4) `OFF` — sekcja opuściła viewport, loop zatrzymany natychmiast. **Przejście OFF → COLD:** po ~30s off-screen (lub przy `document.hidden`) — pełny dispose i zwrot zasobów GPU. **Dispose checklist OFF → COLD:** `geometry.dispose()` dla każdej własnej geometrii; `material.dispose()` dla każdego własnego materiału; `texture.dispose()` dla każdej własnej tekstury i render target texture; `renderer.dispose()` — zwalnia kontekst GL; `renderer.forceContextLoss()` opcjonalnie gdy naprawdę schodzisz do COLD i chcesz agresywnie oddać kontekst systemowi (nie jako obowiązek przy każdym OFF); DOM cleanup canvasa — TYLKO jeśli canvas został stworzony imperatywnie przez runtime (nie gdy canvas należy do markup sekcji — jego usunięcie należy wtedy do normalnego unmount komponentu). **Warmup COLD → WARM:** `requestIdleCallback` z obowiązkowym fallbackiem i `timeout` (API niedostępne w starszych przeglądarkach): `const ric = window.requestIdleCallback ?? ((cb) => setTimeout(() => cb({ didTimeout: false, timeRemaining: () => 50 }), 200))`. Zawsze używaj z `{ timeout: 2000 }` — callback może być opóźniony o wiele sekund bez timeout. **webglBroker odpowiada za:** jeden `getWebGLProfile()` (G11.1), kolejkę warmup GPU (kompilacje sekwencyjnie — nie równolegle na tym samym GPU), centralny handler `webglcontextlost` (log + `pauseAll()` + próba recovery), globalny `pause-all` przy `document.hidden`, limit sekcji HOT jednocześnie (Tier 0: max 1, Tier 1: max 2, Tier 2: bez limitu). **ZAKAZ:** `new THREE.WebGLRenderer()` w sekcji bez przejścia przez stan WARM (renderer tworzony przy near-viewport, nie przy mount); `renderer.setAnimationLoop()` — sekcja używa własnego rAF zarządzanego przez lifecycle; jeden globalny renderer współdzielony między sekcjami — każda sekcja ma własny. **Manifest sekcji WebGL:** `webgl: { shaderComplexity: 'low'|'medium'|'high', textureMemoryMB: number, fallback: 'css'|'static-image'|'hidden' }`. `fallback` obowiązkowy — używany gdy `getWebGLProfile()` zwróci `none` (Tier 0). | Shader compilation stall = freeze 50-200ms przy pierwszej klatce sekcji, szczególnie na Apple GPU (brak precompiled shader cache między sesjami). Stan WARM z `compileAsync()` eliminuje stall przez prekompilację shaderów zanim sekcja wejdzie w viewport. `initTexture()` uploaduje tekstury do GPU bez czekania na pierwszy render — eliminuje texture upload jank. Dispose po ~30s OFF zwalnia kontekst GL i VRAM zanim system zacznie odczuwać pressure (istotne na Mac z iGPU gdzie VRAM = shared RAM). Per-sekcja renderer zachowuje izolację sekcji (Konstytucja B1) — sekcja jest izolowanym bytem, nie zakładnikiem globalnego rendering pipeline. cienki broker daje porządek (wspólny profil, sequential warmup, contextlost recovery) bez budowania nowego imperium. |
...
## CZĘŚĆ VI.19 — PREVIEW Tooling: import policy dla Three.js

**PREVIEW-IMPORT-01** — reguła narzędzia deweloperskiego, nie produkcji.

W produkcji Three.js przychodzi z npm przez bundler — import maps nie istnieją. PREVIEW.html jest narzędziem Fabryki działającym jako standalone plik HTML.

**Import maps wspierane natywnie od Safari 16.4.** Na starszych Safari (13.1–16.3) `<script type="importmap">` jest ignorowany → bare specifier `'three'` nierozwiązywalny → moduł nie startuje → silent failure (brak błędu w konsoli, pusty canvas).

**Wymagana polityka dla PREVIEW.html:**

```
ZAKAZ: <script type="importmap"> w PREVIEW.html
ZAKAZ: es-module-shims jako domyślne rozwiązanie
       (dodatkowy CDN dependency + timing edge case'y z async load)

WYMAGANE: pełne URL-e do Three.js i addonów:
  import * as THREE from 'https://cdn.jsdelivr.net/npm/three@0.160.0/build/three.module.js';
  import { EffectComposer } from 'https://cdn.jsdelivr.net/npm/three@0.160.0/examples/jsm/...';

WYMAGANE: komentarz wersji na górze PREVIEW.html:
  <!-- THREE_VERSION: 0.160.0 — aktualizuj tutaj i w importach poniżej -->
  Zmiana wersji = find/replace na stringu wersji w całym pliku.
```

Działa na Safari 10.1+. Zero dodatkowych zależności. Zero timing issues. Deterministyczne dla narzędzia deweloperskiego.

Ta reguła dotyczy **wyłącznie PREVIEW.html generowanego przez P3**. Produkcja (Next.js/npm) nie używa import maps w żadnym wariancie.
...
| # | Bramka | Kiedy | Kto |
|---|--------|-------|-----|
| B0 | **NODE-VERSION-01 — Node 20.x jako jedyna wersja dev i CI.** Repo MUSI zawierać `.nvmrc` z treścią `20` (opcja A) lub `volta pin node@20` w `package.json` (opcja B). Lokalny build na Node 22/24 = wynik niegwarantowany — tooling i typy mogą zachowywać się inaczej niż Node 20 na Vercelu. Przed każdym `npm run build`: `node --version` musi pokazać `v20.x.x`. **Wdrożenie:** `echo "20" > .nvmrc` w root repo — jednorazowe, raz na projekt. | Przed pierwszym commitem do repo | Integrator |
...
- [ ] **Node 20.x** — `.nvmrc` z `20` w root repo (B0). `node --version` = v20.x.x przed każdym buildem.
- [ ] `globals.css` zawiera `html { font-size: 16px; }` — stałe, jawne (B4.1). Brak = dodaj przed integracją pierwszej sekcji.
- [ ] `SmoothScrollProvider` owija `{children}` w `layout.tsx`
- [ ] `lenis.css` zaimportowany w `layout.tsx` (nie globals.css) — sprawdź ścieżkę wg wersji Lenis (C7). STOP jeśli projekt ma `html { height: 100% }`.
- [ ] `scrollRuntime.ts` z `prioritize: true` na ticker Lenisa
- [ ] `moduleLoader.ts` z warmup policy skonfigurowaną wg manifestów sekcji
- [ ] `orientationchange` listener w `SmoothScrollProvider` z **double rAF settle** (C5)
...
| v2.6 | **Next.js 16 hardening — wynik Master Audit.** NOWE REGUŁY: J13 (GSAP-SSR-01 — registerPlugin wyłącznie w useGSAP lub window guard, nie na module top-level), J14 (GSAP-SSR-02 — escape hatch dla importów z window side-effects: manifest clientOnly + ssr:false, zakaz require() i async IIFE w useGSAP), B4.1 (HTML-FONTSIZE-01 — stałe `html { font-size: 16px }` w globals.css, sekcja NIE deklaruje własnego font-size, responsive przez clamp()), B0-bramka (NODE-VERSION-01 — .nvmrc z Node 20, jedyna wersja dev+CI). AKTUALIZACJE: C5 (orientationchange z double rAF settle — rotacja "osiada" po 1-2 klatkach, implementacja w SmoothScrollProvider), C7 (lenis.css import w layout.tsx nie globals.css + STOP check dla html { height: 100% }). CHECKLIST GLOBALNY: Node 20, html font-size 16px, lenis w layout.tsx, double rAF. ResourceHints: migracja na Client Component z named exports react-dom (preconnect, preload, prefetchDNS) — oficjalna ścieżka Next.js App Router (wdrożone w P4_INTEGRATOR v1.3 i P4_DEVELOPER_GUIDE). |
| v2.7 | **Fabryka hardening — wynik audytu dwóch case'ów produkcyjnych (BookStats + Fakty).** NOWE REGUŁY: J13 rozszerzony o bundler-first zakaz `window.*` jako źródła bibliotek modułowych — wzorzec „w PREVIEW globalne CDN → w TSX window.xxx" jest always-false w bundlerze = najcichszy typ regresu; grep gate w P3. B7.4 (geometry sensitivity contract) — `sensitiveTo` niepuste musi mieć mechanizm ochronny: `refreshSignals` / `stack-harness` / `geometryRefresh: self` / `specialNotes`; BLOCKER w P2B jeśli brak; kluczowa distinkcja: `geometryMutable` (sekcja **zmienia** geometrię) ≠ `sensitiveTo` (sekcja jest **zależna** od geometrii otoczenia) — to nie są antonimy, sekcja może być jedno bez drugiego. C6.3 (async boundary → post-settle requestRefresh) — ST tworzone po async granicy (fonts-ready, assets-loaded, setTimeout, loadedmetadata) są poza oknem pomiarów GSAP; async callback odpala po hydration Next.js = stale coordinates; P2B wpisuje async boundaries do `refreshSignals`; P3 konsumuje mechanicznie; `refreshSignals: []` przy async ST boundary = BLOCKER w P2B. ŹRÓDŁO ZMIAN: case BookStats (`window.gsap` = undefined w bundlerze = animacje never fire bez błędu) + case Fakty (`sensitiveTo` bez mechanizmu = fałszywe poczucie bezpieczeństwa + async `fonts.ready` bez requestRefresh = stale ST coordinates w Next.js). |
| v2.8 | **WebGL Policy — wynik głębokiej analizy wielorundowej (3 konsultantów + adwokat diabła).** NOWE REGUŁY: J15 (WebGL Lifecycle — 4 stany COLD→WARM→HOT→OFF, per-sekcja renderer/canvas, cienki webglBroker, compileAsync+initTexture w WARM, dispose checklist z rozróżnieniem ownership canvasa, requestIdleCallback fallback obowiązkowy, manifest webgl{}). G11.1 (getWebGLProfile — jeden profil jakości dla całej strony: none/low/normal/high, capability tier + opcjonalny hint GPU z WEBGL_debug_renderer_info, zakaz UA/platform heurystyk jako jedynego sygnału, zakaz różnych profili w różnych sekcjach). VI.19/PREVIEW-IMPORT-01 (zakaz import maps w PREVIEW.html — Safari < 16.4 silent failure; wymagane pełne URL-e do three + komentarz THREE_VERSION: x.y.z). SŁOWNIK: webglBroker, WebGL Profile, WebGL Lifecycle. Shared Core Layer: dodano webglBroker. ŹRÓDŁO: "down fale" na Mac — shader compilation stall przy pierwszym wejściu sekcji; eliminowane przez stan WARM z compileAsync() przed viewport. Per-sekcja renderer wybrany nad shared renderer: izolacja sekcji zachowana, złożoność implementacji niska, przy sekcjach WebGL rozdzielonych HTML-em shared renderer nie daje wymiernego zysku. |
| v2.9 | **Section Position Invariance — wynik audytu scroll-trigger positioning.** NOWA REGUŁA: B7.5 (POS-ROOT-01 — trigger ScrollTriggerów opisujących wejście/wyjście/aktywację sekcji musi wskazywać root container; start/end wyłącznie w notacji element↔viewport; zakaz document-relative liczb; nota interpretacyjna: inner trigger dla lokalnych animacji OK, viewport-relative string z px offsetem = REVIEW nie STOP; P3 konsumuje 'section-in-view' i 'layout-settle' mechanicznie z refreshSignals jak inne sygnały; mechanizmy ST-REFRESH-01 zostają w init(container) po translacji — nie useEffect). POWIĄZANIA: B7.4 (sensitiveTo wymaga mechanizmu ochronnego), C6.3 (async boundary → requestRefresh). ŹRÓDŁO: problem sekcji działających poprawnie w PREVIEW (sekcja na górze dokumentu) a sypią się w środku LP (sekcja po 200vh spacerze) — brak reguły która nazywała i egzekwowała viewport-relative semantykę pozycji sekcji. |
